...
import com.jupiter.shortlink.agent.harness.api.AgentChatController;
import com.jupiter.shortlink.agent.harness.runtime.DefaultAgentRunHarness;
...
import com.jupiter.shortlink.agent.infrastructure.persistence.JdbcGraphCheckpointStore;
...
class AgentChatE2eTest {
    ...
    void chatRunsGraphToolsAdminInternalApiAndCheckpointEndToEnd() throws Exception {
        ...
        ShortLinkBusinessHttpGateway businessGateway =
                new ShortLinkBusinessHttpGateway(properties, restTemplate);
        DefaultCampaignAnalysisGraphExecutor graphExecutor =
        ...
        MockMvc mockMvc =
                MockMvcBuilders.standaloneSetup(
                                new AgentChatController(
                                        new DefaultAgentRunHarness(
                                                graphExecutor,
                                                request -> {
                                                    throw new AssertionError(
                                                            "Default E2E chat should route to"
...
"""))
                        ...
                        .andExpect(
                                jsonPath("$.data.toolCalls[3].name")
                                        .value("get_group_access_records"))
                        ...
                        .andExpect(
                                jsonPath("$.data.traceEvents[2].nodeName").value("insight_compute"))
                        .andExpect(jsonPath("$.data.traceEvents[3].nodeName").value("llm_analysis"))
                        .andExpect(
                                jsonPath("$.data.traceEvents[4].nodeName")
                                        .value("response_compose"))
                        .andExpect(
                                jsonPath("$.data.traceEvents[5].nodeName").value("checkpoint_save"))
    ...
    @Test
    void chatRoutesSecurityRiskAgentThroughAdminInternalApiAndCheckpointEndToEnd()
            throws Exception {
        AgentProperties properties = agentProperties();
        RestTemplate restTemplate = new RestTemplate();
        MockRestServiceServer adminServer = MockRestServiceServer.createServer(restTemplate);
        JdbcTemplate jdbcTemplate = new JdbcTemplate(h2DataSource());
        JdbcGraphCheckpointStore checkpointStore = new JdbcGraphCheckpointStore(jdbcTemplate);
        CapturingLlmChatClient llmChatClient =
                new CapturingLlmChatClient("E2E security risk answer");
        ShortLinkBusinessHttpGateway businessGateway =
                new ShortLinkBusinessHttpGateway(properties, restTemplate);
        DefaultSecurityRiskGraphExecutor securityRiskGraphExecutor =
                ...
                MockMvcBuilders.standaloneSetup(
                                new AgentChatController(
                                        new DefaultAgentRunHarness(
                                                request -> {
                                                    throw new AssertionError(
                                                            "security-risk E2E chat should not"
                                                                + " route to campaign analysis"
                                                                + " agent");
                                                },
...
"""))
                        ...
                        .andExpect(
                                jsonPath("$.data.toolCalls[1].name")
                                        .value("get_group_access_records"))
                        .andExpect(jsonPath("$.data.cards[0].type").value("risk_signal"))
                        .andExpect(
                                jsonPath("$.data.cards[0].summary.reasonCode")
                                        .value("top_ip_concentration"))
                        .andExpect(
                                jsonPath("$.data.pendingActions[0].type")
                                        .value("review_security_risk"))
                        .andExpect(
                                jsonPath("$.data.dataSources[0].name").value("security-risk-graph"))
                        .andExpect(jsonPath("$.data.dataSources[2].type").value("tool"))
                        .andExpect(jsonPath("$.data.traceEvents[0].nodeName").value("intake"))
                        .andExpect(
                                jsonPath("$.data.traceEvents[1].nodeName")
                                        .value("profile_candidate_load"))
                        .andExpect(
                                jsonPath("$.data.traceEvents[2].nodeName")
                                        .value("risk_tool_planning"))
                        .andExpect(jsonPath("$.data.traceEvents[3].nodeName").value("risk_scoring"))
                        .andExpect(
                                jsonPath("$.data.traceEvents[4].nodeName").value("llm_explanation"))
                        .andExpect(
                                jsonPath("$.data.traceEvents[5].nodeName")
                                        .value("risk_event_persist"))
                        .andExpect(
                                jsonPath("$.data.traceEvents[6].nodeName")
                                        .value("risk_auto_action"))
                        .andExpect(
                                jsonPath("$.data.traceEvents[7].nodeName")
                                        .value("response_compose"))
                        .andExpect(
                                jsonPath("$.data.traceEvents[8].nodeName").value("checkpoint_save"))
    ...
    private DataSource h2DataSource() {
        ...
        dataSource.setUrl(
                "jdbc:h2:mem:agent_chat_e2e;MODE=MySQL;DATABASE_TO_UPPER=false;DB_CLOSE_DELAY=-1");
    ...
    private void expectListGroups(MockRestServiceServer adminServer) {
        adminServer
                .expect(
                        requestTo(
                                "http://admin.test/internal/short-link-admin/v1/agent-tools/groups"))
    ...
    private void expectPageShortLinks(MockRestServiceServer adminServer) {
        adminServer
                .expect(
                        requestTo(
                                "http://admin.test/internal/short-link-admin/v1/agent-tools/short-links/page?gid=g1&current=1&size=10"))
                .andExpect(method(HttpMethod.GET))
                .andExpect(header("X-Agent-Internal-Token", "internal-token"))
                .andExpect(header("X-Agent-Username", "trusted-user"))
                .andExpect(headerDoesNotExist("username"))
    ...
    private void expectGroupStats(MockRestServiceServer adminServer) {
        adminServer
                .expect(
                        requestTo(
                                "http://admin.test/internal/short-link-admin/v1/agent-tools/group/stats?gid=g1&startDate=2026-07-01&endDate=2026-07-07"))
                .andExpect(method(HttpMethod.GET))
                .andExpect(header("X-Agent-Internal-Token", "internal-token"))
                .andExpect(header("X-Agent-Username", "trusted-user"))
                .andExpect(headerDoesNotExist("username"))
    ...
    private void expectGroupAccessRecords(MockRestServiceServer adminServer) {
        adminServer
                .expect(
                        requestTo(
                                "http://admin.test/internal/short-link-admin/v1/agent-tools/group/access-records?gid=g1&startDate=2026-07-01&endDate=2026-07-07&current=1&size=10"))
                .andExpect(method(HttpMethod.GET))
                .andExpect(header("X-Agent-Internal-Token", "internal-token"))
                .andExpect(header("X-Agent-Username", "trusted-user"))
                .andExpect(headerDoesNotExist("username"))
