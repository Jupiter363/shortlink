 ...
 *
 * <p>The model bean is deliberately lazy with respect to the API key: the application can start
 * without a provider secret and retains the previous request-time {@code
 * LlmApiKeyNotConfiguredException} behavior.
 */
@Configuration
public class SpringAiChatConfig {
    ...
    public ChatModel agentChatModel(
            DeepSeekProperties properties, RestTemplate deepSeekRestTemplate) {
    ...
    public ChatClient agentChatClient(
            ChatModel agentChatModel, ToolCallbackProvider agentToolCallbackProvider) {
        return ChatClient.builder(agentChatModel)
                .defaultOptions(
                        agentChatModel.getDefaultOptions() instanceof ToolCallingChatOptions options
                                ? options
                                : ToolCallingChatOptions.builder().build())
                .defaultAdvisors(new BoundedToolCallAdvisor(8))
                .defaultToolCallbacks(agentToolCallbackProvider)
                .build();
    }

    /**
     * Explanation nodes use a callback-free client so a model response cannot start an implicit
     * tool/ReAct loop. Tool-capable callers use the primary {@code agentChatClient} above and
     * explicitly provide trusted context.
     */
