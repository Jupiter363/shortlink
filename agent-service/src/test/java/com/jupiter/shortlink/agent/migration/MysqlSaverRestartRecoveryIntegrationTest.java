...
/**
 * Runs against a real MySQL instance when AGENT_TEST_MYSQL_URL is supplied. MysqlSaver owns its
 * tables and therefore this contract intentionally checks recovery through a newly-created saver
 * instance (the same path used after a process restart), rather than relying on MemorySaver state.
 */
class MysqlSaverRestartRecoveryIntegrationTest {
    ...
    void checkpointSurvivesSaverRecreationAndGraphVersionRemainsIsolated() throws Exception {
        ...
        Optional<Checkpoint> restored =
                afterRestartSaver.get(RunnableConfig.builder().threadId(v1Thread).build());
        Optional<Checkpoint> incompatibleVersion =
                afterRestartSaver.get(RunnableConfig.builder().threadId(v2Thread).build());
