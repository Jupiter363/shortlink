...
class RiskPolicyRepositoryTest {
    ...
    @Test
    void agentRetainsAuditButCannotOwnPolicyFacts() {
        ...
        assertThatThrownBy(() -> policyRepository.saveActive(policy))
                .isInstanceOf(SecurityException.class);
        auditRepository.saveActivationAudit(policy, "manual-user", "confirmed high risk");

        assertThatThrownBy(() -> policyRepository.findByPolicyId("policy-001"))
                .isInstanceOf(SecurityException.class);
        assertThat(auditRepository.countByPolicyId("policy-001")).isEqualTo(1);

        assertThatThrownBy(() -> policyRepository.disable("policy-001", "trace-002"))
                .isInstanceOf(SecurityException.class);
        assertThat(auditRepository.countByPolicyId("policy-001")).isEqualTo(1);
    }

    @Test
    void legacyPolicyLookupAndMutationFailClosed() {
        ...
        assertThatThrownBy(() -> policyRepository.saveActive(policy))
                .isInstanceOf(SecurityException.class);
        assertThatThrownBy(() -> policyRepository.expire("policy-expired-001", "trace-002"))
                .isInstanceOf(SecurityException.class);

        assertThatThrownBy(() -> policyRepository.findByPolicyId("policy-expired-001"))
                .isInstanceOf(SecurityException.class);
        assertThatThrownBy(() -> policyRepository.findActiveByPolicyKey(policy.policyKey()))
                .isInstanceOf(SecurityException.class);
    }
