...
- 依赖：无；以用户确认的首次上线范围为准。
- 拥有：`plan/生产级重构增强/decisions/capacity-and-scope.md`（planned）。
...
- 拥有：`event-contract/`（planned），`plan/生产级重构增强/decisions/contracts.md`（planned）。
...
- 拥有：`scripts/perf/`（planned，排除 M3-04 的 `batch/`），`docs/performance/`（planned）；基础脚本完成后按公共冲突表交接 M6-02。
...
- 依赖：M0-02 的模块间契约。
- 拥有：`plan/生产级重构增强/decisions/modules-and-build.md`（planned）；公共构建文件仅由主集成人修改。
...
- 拥有：`admin/src/main/java/com/jupiter/shortlink/admin/dto/req/analytics/`、`admin/src/main/java/com/jupiter/shortlink/admin/dto/resp/analytics/`、`admin/src/test/java/com/jupiter/shortlink/admin/analyticscontract/`、`plan/生产级重构增强/decisions/agent-analytics-contract.md`（planned）。Kafka 事件/schema 仍归 M0-02；其他模块只遵循线协议与样例测试，不依赖这些 Admin 内部实体。
...
- 控制库与证据拥有：`deploy/mysql/002-analytics-control-schema.sql`、`scripts/analytics/`、`docs/analytics/`（planned）。002 在独立统计控制库定义归档范围、处理覆盖、任务/租约/fencing、recoveryEpoch、构建版本、发布 manifest/revision；不与 Agent DB 或业务 ds_0 共用本地事务，不归 M5-06。
...
- 拥有：`deploy/runtime/`、`deploy/observability/`、`docs/operations/`（planned）；模块启动配置由主集成人接入。
...
- 依赖：M1～M5 功能交付。
- 拥有：`scripts/e2e/production-readiness/`、`docs/acceptance/functional-security.md`（planned）。
...
- 拥有：M0-03 已交接的 `scripts/perf/` 最终场景编排、`docs/acceptance/performance.md`（planned），排除 M3-04 的 `scripts/perf/batch/`；只调用其脚本，修改需求交原 owner。
...
- 拥有：`scripts/resilience/`、`docs/acceptance/resilience.md`（planned）。
...
- 拥有：`docs/release/first-production-release.md`、`deploy/release/`（planned）。
