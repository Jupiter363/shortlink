...
Derived insight context 进入 prompt 前也必须经过统一脱敏；
存在派生洞察卡片时，prompt 必须追加 Insight explanation contract，约束模型输出可能原因、风险级别、证据引用和建议动作；
Insight explanation contract 必须要求模型不得重新计算、编造或覆盖 cards 中的 metrics、thresholds、evidence、type、sourceTool、reasonCode；
