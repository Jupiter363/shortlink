...
```text
EXECUTED：200，返回已经保存的 resultJson。
...
保存 rejectionReason。
可选 reviewAction 只允许 IGNORE 或 FALSE_POSITIVE。
...
最大尝试次数：可配置，默认 10。
超过上限：DEAD。
DEAD 支持内部 replay API，并记录操作人和审计。
```
...
GET  /internal/short-link-agent/v1/actions/{actionId}
...
重复确认 `EXECUTED` 动作返回 200 和原执行结果，不返回冲突。
...
BLOCK_IP 的原始 IP 只允许在获得 Tool 结果到计算 hash 的当前内存调用栈中存在。
...
5. legacy idempotencyKey = legacy:{policyId}。
6. 按 policyKey 分组，以 effective_time、id 升序为全部历史记录分配连续 policyVersion。
7. 对每个 policyKey 选择 effective_time 最新、id 最大的 ACTIVE 记录。
8. 将其他 ACTIVE 记录标记为 SUPERSEDED。
9. 使用选中记录的最大 policyVersion 创建 effective slot。
10. 为有效槽位创建 UPSERT outbox，使部署后主动校准 Redis。
11. 将新增列改为 NOT NULL 并添加唯一键。
12. 写 migration history。
```
...
```text
1. 暂停 risk-profile scheduler、risk-analysis worker 和旧策略写入口。
2. 备份相关表并记录行数。
3. 执行迁移。
4. 部署 agent-service。
5. 启动 outbox worker，确认 legacy UPSERT 被处理。
6. 部署 gateway 短链接范围 BLOCK_IP 兼容逻辑。
7. 部署 admin API 和页面。
8. 恢复风险任务。
```
