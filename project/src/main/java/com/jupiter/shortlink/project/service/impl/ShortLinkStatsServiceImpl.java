...
import com.jupiter.shortlink.project.dto.req.ShortLinkGroupStatsAccessRecordReqDTO;
import com.jupiter.shortlink.project.dto.req.ShortLinkGroupStatsReqDTO;
import com.jupiter.shortlink.project.dto.req.ShortLinkStatsAccessRecordReqDTO;
import com.jupiter.shortlink.project.dto.req.ShortLinkStatsReqDTO;
import com.jupiter.shortlink.project.dto.resp.*;
import com.jupiter.shortlink.project.service.ShortLinkStatsService;
...
// 预处理：建立日期字符串到统计数据的映射
        Map<String, LinkAccessStatsDO> dateToStatsMap = listStatsByGroup.stream()
                .collect(java.util.stream.Collectors.toMap(
                        item -> DateUtil.formatDate(item.getDate()),
                        item -> item,
...
// 简化循环逻辑
        rangeDates.forEach(each -> {
            LinkAccessStatsDO statsDO = dateToStatsMap.get(each);
            ShortLinkStatsAccessDailyRespDTO accessDailyRespDTO = ShortLinkStatsAccessDailyRespDTO.builder()
                    .date(DateUtil.parse(each))  // 关键修改：将字符串转换为Date
                    .pv(statsDO != null ? statsDO.getPv() : 0)
                    .uv(statsDO != null ? statsDO.getUv() : 0)
                    .uip(statsDO != null ? statsDO.getUip() : 0)
                    .build();
            daily.add(accessDailyRespDTO);
        });
        // 地区访问详情（仅国内）
