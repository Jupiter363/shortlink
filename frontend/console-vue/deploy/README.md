...
The file contains no credentials or internal token. Browser authentication remains the `Username` and `Token` headers sent by the application. Relay logout sends no query or body credentials, so upstream error logs cannot capture a token in that request URL. Deploy the compatible Admin header-only logout change before the new client. The access log also records `$uri` without arguments for older clients; this alone does not redact query parameters from error logs. Retained legacy clients should migrate to header-only logout.
