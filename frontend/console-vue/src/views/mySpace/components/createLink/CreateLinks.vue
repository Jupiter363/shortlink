...
import { createRequestId, responseErrorMessage } from '@/utils/requestId.js'

const store = useStore()
const defaultDomain = store.state.domain ?? null
...
const formData = reactive({
  requestId: createRequestId('create-batch'),
...
const initFormData = () => {
  formData.requestId = createRequestId('create-batch')
...
watch(
  () => props.groupInfo,
  (nV) => {
    groupInfo.value = Array.isArray(nV) ? nV : []
    // console.log('默认的gid', props.defaultGid)
    formData.gid = props.defaultGid || groupInfo.value[0]?.gid || null
  },
  {
    immediate: true
  }
...
watch(
  ...
  () => {
    if (props.defaultGid) {
      ...
      formData.gid = groupInfo.value[0]?.gid ?? null
    }
  },
  {
    immediate: true
...
const onSubmit = async (formEl) => {
  ...
  await formEl.validate(async (valid) => {
    ...
    try {
      ...
      } else if (res.data.data?.jobId) {
        ElMessage.success(`批量任务已提交：${res.data.data.jobId}`)
        emits('onSubmit', false)
      } else {
        ElMessage.success(`已创建 ${res.data.data?.total ?? originUrls.length} 条短链接`)
        emits('onSubmit', false)
      }
    } catch (error) {
      ElMessage.error(responseErrorMessage(error, '批量创建失败'))
    } finally {
      submitDisable.value = false
    }
  })
