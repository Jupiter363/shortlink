...
const formData = reactive({
  requestId: createRequestId('create-link'),
...
const initFormData = () => {
  formData.requestId = createRequestId('create-link')
...
const queryTitle = (url) => {
  if (reg.test(url)) {
    ...
    API.smallLinkPage.queryTitle({ url: url })
      .then((res) => {
        if (formData.originUrl === url && !formData.describe) {
    ...
    try {
      ...
      if (!res?.data?.success) {
        if (res?.data?.code === 'A000001') {
          ElMessage.warning({ message: res.data.message, duration: 5000 })
        } else {
      ...
      ElMessage.error(responseErrorMessage(error, '创建失败'))
    } finally {
      submitDisable.value = false
    }
  })
