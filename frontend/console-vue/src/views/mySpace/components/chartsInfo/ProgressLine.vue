<template>
  <div class="main-box">
    <span v-if="!dataLists?.length"> 暂无该维度的已知数据 </span>
    <div v-else>
      <div class="flex-box" v-for="(item, index) in dataLists" :key="index">
        <div class="flex-item">
          <div>
            <img v-if="showIcon" :src="getUrl(item?.browser, item?.os)" width="25" alt="" />
            <span>{{ labelField ? item?.[labelField] : (item?.browser || item?.os) }} {{ (item?.ratio * 100).toFixed(2) }}%</span>
          </div>
          <div>
