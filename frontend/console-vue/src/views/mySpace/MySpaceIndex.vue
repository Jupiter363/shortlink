<template>
  <div style="display: flex; height: 100%">
      ...
      <ul class="sortOptions">
        <li v-for="(item, index) in editableTabs" :key="item.gid">
          <div class="item-box flex-box hover-box" :class="{ selectedItem: selectedIndex === index }"
            ...
            <div class="flex-box">
              <!-- 图标 -->
              <el-tooltip v-if="!(item.shortLinkCount === 0 || item.shortLinkCount === null)" show-after="500"
                class="box-item" effect="dark" content="查看图表" placement="bottom-end">
                <!-- 传group是为了表示这个请求是查询分组图表数据 -->
                <el-icon class="edit" role="button" tabindex="0" :aria-label="`查看分组统计：${item.name}`"
                  @keydown.enter="chartsVisible({ describe: item.name, gid: item.gid, group: true })"
                  @click="chartsVisible({ describe: item.name, gid: item.gid, group: true })">
                  <Histogram />
                </el-icon>
              </el-tooltip>
              <!-- 编辑按钮 -->
              <el-dropdown v-if="item.name !== '默认分组'">
                <div class="block">
                  <el-icon class="edit">
                    <Tools />
                  </el-icon>
          ...
          <el-table-column label="短链接网址" prop="url" min-width="300">
            <template #default="scope">
              <div class="table-url-box">
                <!-- 当失效后就不能在点击跳转了 -->
                <el-link type="primary" underline="never" target="_blank"
                  :disabled="scope?.row?.validDateType === 1 && !isExpire(scope?.row?.validDate)"
                  :href="publicShortUrl(scope.row)">{{ scope.row.domain + '/' + scope.row.shortUri }}</el-link>
          ...
          <el-table-column prop="copy" width="170">
            <template #default="scope">
              <div style="display: flex; align-items: center">
                <!-- 二维码 -->
                <QRCode :url="publicShortUrl(scope.row)"></QRCode>
                <!-- 表格中的复制链接按钮 -->
                <el-tooltip show-after="500" class="box-item" effect="dark" content="复制链接" placement="bottom-end">
                  <el-icon @click="copyUrl(publicShortUrl(scope.row))" class="table-edit copy-url">
                    <Share />
                  </el-icon>
                </el-tooltip>
              </div>
            ...
            <template #default="scope">
              ...
              <el-link :underline="false" class="el-link" type="primary">编辑</el-link> -->
                ...
                <el-tooltip show-after="500" class="box-item" effect="dark" content="查看图表" placement="bottom-end">
                  <el-icon class="table-edit" role="button" tabindex="0" :aria-label="`查看短链统计：${scope.row.describe || scope.row.fullShortUrl}`" @keydown.enter="chartsVisible(scope.row)" @click="chartsVisible(scope.row)">
                    <Histogram />
                  </el-icon>
                </el-tooltip>
                <!-- 正常页面展示编辑和删除 -->
    ...
    <!-- 查看数据弹框 -->
    <ChartsInfo v-if="isChartsMounted" style="width: 880px" ref="chartsInfoRef" :title="chartsInfoTitle" :info="chartsInfo"
      :tableInfo="tableInfo" :isGroup="isGroup" :nums="nums" :favicon="favicon1" :originUrl="originUrl1"
      @changeTime="changeTime" @changePage="changePage" @closed="closeStatistics" top="60px"></ChartsInfo>
    <!-- 新建分组弹框 -->
    <el-dialog v-model="isAddGroup" title="新建短链接分组" style="width: 40%">
      <el-form>
        <el-form-item label="分组名称：" label-width="110px">
...
<script setup>
import { defineAsyncComponent, getCurrentInstance, nextTick, onBeforeUnmount, onMounted, reactive, ref, watch } from 'vue'
...
import { adaptAnalyticsStats, AnalyticsStatsError, createStatsCursorPager } from '@/utils/analyticsStats.js'

const ChartsInfo = defineAsyncComponent(() => import('./components/chartsInfo/ChartsInfo.vue'))
...
const publicShortUrl = (row) => {
  const address = row?.fullShortUrl
  if (!address) return ''
  if (/^https?:\/\//i.test(address)) return address
  const localHost = ['localhost', '127.0.0.1'].includes(window.location.hostname)
  return `${localHost ? 'http' : 'https'}://${address}`
}
...
const recordsPager = createStatsCursorPager()
let statisticsRevision = 0
let statisticsContext = null
let pendingChartOpen = null

const openStatisticsWhenReady = () => {
  const pending = pendingChartOpen
  if (!pending || !statisticsContext || pending.context !== statisticsContext
      || pending.revision !== statisticsRevision || typeof chartsInfoRef.value?.isVisible !== 'function') return
  pendingChartOpen = null
  chartsInfoRef.value.isVisible()
}

// The async component can finish loading several ticks after the first statistics click.
watch(chartsInfoRef, openStatisticsWhenReady, { flush: 'post' })

const setStatsDates = (dateList) => {
  if (!dateList) {
    initStatsFormData()
  } else {
...
const loadRecordPage = async (current) => {
  const context = statisticsContext
  const revision = statisticsRevision
  if (!context) return
  tableInfo.value = recordsPager.view({ loading: true, error: '' })
  try {
    const result = await recordsPager.load(current, (params) => context.group
      ? API.group.queryGroupTable(params)
      : API.smallLinkPage.queryLinkTable(params))
    if (result && statisticsContext === context && statisticsRevision === revision) {
      tableInfo.value = result
      statsFormData.current = result.data.data.current
    }
  } catch (error) {
    if (statisticsContext === context && statisticsRevision === revision) {
      tableInfo.value = recordsPager.view({
        loading: false,
        error: error instanceof AnalyticsStatsError ? error.message : '访问记录加载失败，请重试或重新选择日期'
      })
    }
  }
}
...
const reloadStatistics = async () => {
  ...
  // Capture scope and dates before either asynchronous request starts.
  const context = { group: Boolean(isGroup.value), params: { ...statsFormData, ...visitLink } }
  statisticsContext = context
  recordsPager.reset(context.params, statsFormData.size)
  chartsInfo.value = { loading: true, error: '', meta: {}, dimensionQuality: {} }
  const summary = async () => {
    try {
      const response = context.group
        ? await API.group.queryGroupStats({ ...context.params })
        : await API.smallLinkPage.queryLinkStats({ ...context.params })
      if (statisticsRevision === revision && statisticsContext === context) {
        chartsInfo.value = adaptAnalyticsStats(response)
      }
    } catch (error) {
      if (statisticsRevision === revision && statisticsContext === context) {
        const message = error instanceof AnalyticsStatsError ? error.message : '统计加载失败，请稍后重试'
        chartsInfo.value = { loading: false, error: message, meta: {}, dimensionQuality: {} }
        ElMessage.error(message)
      }
    }
  }
  await Promise.allSettled([summary(), loadRecordPage(1)])
}
...
const chartsVisible = async (rowInfo, dateList) => {
  ...
  isGroup.value = Boolean(group)
  Object.assign(visitLink, { fullShortUrl, gid, enableStatus })
  setStatsDates(dateList)
  isChartsMounted.value = true
  const loading = reloadStatistics()
  pendingChartOpen = { revision: statisticsRevision, context: statisticsContext }
  await nextTick()
  openStatisticsWhenReady()
  await loading
}
...
const changePage = async (page) => {
  if (!statisticsContext) return
  const size = page.size ?? statsFormData.size
  if (size !== statsFormData.size) {
    recordsPager.reset(statisticsContext.params, size)
    statsFormData.size = size
    statsFormData.current = 1
    await loadRecordPage(1)
    return
  }
  await loadRecordPage(page.current ?? 1)
}
...
const closeStatistics = () => {
  ...
  recordsPager.reset({}, statsFormData.size)
  tableInfo.value = recordsPager.view()
  chartsInfo.value = undefined
}
onBeforeUnmount(closeStatistics)
...
const toRecycleBin = (data) => {
  ...
  API.smallLinkPage
    .toRecycleBin({ gid, fullShortUrl, expectedVersion: data.routeVersion })
    .then((res) => {
      if (res?.data?.code !== '0') {
        ElMessage.error(res.data.message)
      } else {
...
const recoverLink = (data) => {
  const { gid, fullShortUrl } = data
  API.smallLinkPage
    .recoverLink({ gid, fullShortUrl, expectedVersion: data.routeVersion })
    .then(() => {
...
const removeLink = (data) => {
  const { gid, fullShortUrl } = data
  API.smallLinkPage
    .removeLink({ gid, fullShortUrl, expectedVersion: data.routeVersion })
    .then((res) => {
      if (res?.data?.code !== '0') {
        ElMessage.error(res.data.message)
      } else {
