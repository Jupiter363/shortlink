package com.jupiter.shortlink.admin.service;

import com.baomidou.mybatisplus.extension.service.IService;
import com.jupiter.shortlink.admin.account.AccountInitializationStatus;
import com.jupiter.shortlink.admin.dao.entity.UserDO;
import com.jupiter.shortlink.admin.dto.req.UserLoginReqDTO;
import com.jupiter.shortlink.admin.dto.req.UserRegisterReqDTO;
import com.jupiter.shortlink.admin.dto.req.UserUpdateReqDTO;
import com.jupiter.shortlink.admin.dto.resp.UserLoginRespDTO;
import com.jupiter.shortlink.admin.dto.resp.UserRespDTO;

/** 用户接口层 */
public interface UserService extends IService<UserDO> {

    AccountInitializationStatus initializationStatus();

    AccountInitializationStatus retryInitialization();

    /**
     * @param username 用户名
     * @return 用户返回实体
     */
    UserRespDTO getUserByUsername(String username);

    /**
     * 查询用户名是否存在
     *
     * @param username 用户名
     * @return 用户名存在返回true,反之false
     */
    Boolean hasUsername(String username);

    /**
     * 注册用户
     *
     * @param requestParam 注册用户请求参数
     */
    void register(UserRegisterReqDTO requestParam);

    /**
     * 根据用户名修改用户
     *
     * @param requestParam 更新用户请求参数
     */
    void update(UserUpdateReqDTO requestParam);

    /**
     * 用户登录
     *
     * @param requestParam 用户登录请求参数
     * @return 用户登录返回参数 Token
     */
    UserLoginRespDTO login(UserLoginReqDTO requestParam);

    /**
     * 用户退出登录
     *
     * @param username 用户退出请求参数
     * @param token 用户退出请求参数
     */
    void logout(String username, String token);

    /**
     * 检查用户是否登录
     *
     * @param token 用户登录token
     * @return 用户是否登录标识
     */
    Boolean checkLogin(String username, String token);
}
