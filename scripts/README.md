...
三个 `.ps1` 入口面向 Windows PowerShell / PowerShell on Windows，要求 Java 17、Maven，以及已准备好的专用测试依赖。`-JavaHome` 可显式指定 Windows JDK，也可使用 `JAVA_HOME`；Gateway / Redirect 入口会在设置环境和写日志前检查非空值及 `bin/java.exe` 存在，但不代替 Java 版本核对。
...
| [run-gateway-redirect-it.ps1](integration/run-gateway-redirect-it.ps1) | Java 17；匹配专用测试拓扑的本机 Redis 16379、Kafka 19092、MySQL 13306，以及脚本中的测试口令；Maven 在 PATH 可用 | Gateway / Redirect 及 reactor 依赖测试，测试会操作隔离资源；`.work/component-results/gateway-redirect-it-<时间>.log`；此入口没有 `-AllowReset` 参数，不代表无数据副作用 |
...
./scripts/integration/run-gateway-redirect-it.ps1 -JavaHome $env:JAVA_HOME
```
...
| 文件 | 用途与边界 |
| --- | --- |
| [run_create_redirect_e2e.py](e2e/run_create_redirect_e2e.py) | Linux 专用测试环境的监督入口；需要已构建 Command / Admin / Gateway / Redirect JAR、APISIX 与隔离依赖；显式 `--allow-test-database` 后创建测试 schema / Redis 占用标记、启动应用及 APISIX，生成 state 与私有配置 |
...
| [gateway_security_cases.py](e2e/gateway_security_cases.py) | 网关可信头、认证及接入用例，被验证入口调用；需与对应测试 state / 运行配置一致 |
...
| [supervisor.py](performance/supervisor.py) | 专用 Linux / WSL 环境的启动与清理监督；准备测试库、选择空 Redis DB、创建 APISIX 和启动 4 个 Java 服务，输出 `.work/performance/<runId>/state.json` |
...
- [risk-profile-policy-e2e.ps1](risk-profile-policy-e2e.ps1) **仍能发送请求**，使用旧链路与身份协议；它只是历史联调脚本，不是当前策略 / Agent 拓扑的验收入口，也没有被技术性禁用。不要把它当作只读检查或直接套到当前环境。
- `.work` 和 `doc/压测报告/` 中的历史结果、FAIL、运行配置、source SHA、JAR SHA、fixture 和 Kafka 边界保持原样。目录迁移用新的路径映射记录解释旧位置，不批量替换原 JSON 或追改已有验收。
- 活跃模块 / JAR 路径变动时更新当前启动器及导航；服务名、端口、主题、统计契约和资源预算不因目录分组而改名或放宽。
