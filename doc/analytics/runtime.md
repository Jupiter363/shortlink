...
From the repository root, run unit tests with Java 17 and UTF-8: `mvn -pl :event-contract,:analytics-flink,:analytics-worker,:shortlink-analytics-api -am test`. These modules now reside under `libraries/`, `jobs/`, and `services/`; artifactIds remain unchanged. Keep `-am` so the current reactor supplies shared dependencies instead of relying on previously installed local SNAPSHOTs.

Explicit component integration from the repository root: `mvn -pl :analytics-worker,:shortlink-analytics-api -am -Pintegration verify`. The disposable stack uses Kafka `19092`, MySQL `13306/shortlink_analytics_control_it`, ClickHouse `18123/shortlink_analytics_it`, MinIO `19000`, and replicas `18124/18125/shortlink_replicated_it`. Apply the 002 control schema and all relevant ClickHouse schemas first. Initialize the Kafka topics using the repository integration fixture. Credentials are the isolated fixture credentials, never development database credentials.
...
`QueryJobMySqlIntegrationTest` additionally uses a separate, initially empty control fixture so its deliberate singleton-epoch admission test cannot interfere with Worker tests. Set `SHORTLINK_QUERY_TEST_JDBC_URL` to `jdbc:mysql://127.0.0.1:3306/shortlink_analytics_control_it?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC`, `SHORTLINK_QUERY_TEST_USER`, `SHORTLINK_QUERY_TEST_PASSWORD`, and `SHORTLINK_QUERY_TEST_ALLOW_INIT=true` only for the newly provisioned native test database. That fixture cleans its own epoch, window and job rows. Missing explicit initialization authorization fails the test instead of skipping. To select only the Docker-backed API integration cases, use `mvn -pl :shortlink-analytics-api -am -Pintegration verify -Dit.test=AnalyticsSnapshotIntegrationTest,JobClickHouseIntegrationTest,ApiApplicationIntegrationTest -Dfailsafe.failIfNoSpecifiedTests=false` from the repository root. The final flag permits dependency modules without those named cases; verify that all selected API cases actually execute.

Flink's `FlinkKafkaIntegrationTest` uses a real MiniCluster, RocksDB and Kafka. Run on Linux; a missing Windows native DLL is an environment failure, never a substitute for successful integration. For the current layout, use a fresh repository candidate that preserves the root POM and `services/`, `libraries/`, `jobs/`, `scripts/`, and `deploy/` paths. With the isolated Kafka environment configured, run `mvn -pl :analytics-flink -am -Pintegration verify` from that candidate root; do not reuse a pre-migration module `target`.

### Historical verification before the directory migration

The following WSL command and result locations describe the original verified layout. Their paths are preserved as historical evidence, not current copy/build instructions. The verified WSL command, after copying the root/module POMs plus `analytics-flink/src` and `event-contract/src` to `/tmp/shortlink-analytics-test`, was:
