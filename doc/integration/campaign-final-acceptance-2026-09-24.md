...
后续增量：本文记录的是 PR #200 批次的历史快照。浏览器文件、UNKNOWN、权限变化及顺序故障接管后续由 #201 补验，最新完成矩阵见[最终在途验收](campaign-issue201-inflight-2026-09-24.md)；保留下面当时的“待验”原文，避免把后续证据混入旧报告版本。

**结论：本轮复合分析公共主链、真实局部 ReAct、所选后端用例及 1600px／390px 页面复验已通过；浏览器下载文件仍待验。** 复合 Run `campaign-run-b9c90398…15ea0` 已完成 **6/6 步骤**，**7 个唯一异步 query job 全部 SUCCEEDED，attempts 均为 1**，下降集合真实传入两期省份与设备联合下钻；报告 `report-a54e1d81…c2a7b5d` 的 **revision 2 为 READY、3/3 目标 ANSWERED**。另一个单日只读探索 Run `campaign-run-dc4fe737…16cf2` 实际冻结为 `REACT`，完成两轮模型、一次统计工具调用，报告 **2/2 目标 ANSWERED**。早期需求解释、规划准入、冻结查询 DTO 及恢复错误的中间记录仍保留，不作为当前运行结果。

代码修复已进入本轮真实链路；后端结论限于下列定向测试及现场场景。前端维度表直接展示原始 JSON 的问题已修复，并经内置浏览器 1600px 宽屏及 390px 窄屏复验。模型文字中的“两组卡方”错词、UNKNOWN 地域数据，以及尚未执行的真实撤权和多实例故障验收另列限制。本文不据此宣布所有能力或所有视口均已验收。
...
| 浏览器导出下载 | 已点击真实页面导出按钮；内置浏览器网络记录显示所选 ReAct 报告 revision 1 的导出请求 **HTTP 200**，但**未捕获下载文件** | 尚不能证明浏览器落盘文件的字节、内容、文件名或哈希与服务端一致；这次 200 不代替上述复合报告 revision 2 的服务端哈希 | 浏览器文件待验 |
| 继续与报告视觉 | 浏览器已验证继续沿用原问题、分析记录不重复；**1600px** 下维度表显示“**设备 Desktop／省份 广东省**”、基期 **PV 3／UV 2／UIP 2**、目标期 **PV 0／UV 0／UIP 0** 及 PV 占比；复杂原字段可在“完整字段”展开。**390px** 下报告、历史弹窗与同一维度表可读，表格可横向滚动查看后续列 | 主代理在真实内置浏览器复验；390px 页面 `body.scrollWidth=390`，历史弹窗无页面级横向溢出，维度表 270px 容器局部横滚。局部 ESLint／Prettier 及 `npm run build` 已通过；只覆盖这两个视口和本轮数据 | 所选宽屏／窄屏与交互通过 |
...
| 前端恢复、继续或报告投影无法发布 | 对应构建产物能够生成；本轮维度表再经局部 ESLint／Prettier、`npm run build` 和 1600px／390px 内置浏览器复验 | `frontend-recovery-build.log`、`frontend-session-history-build.log`、`frontend-continuation-build.log`、`frontend-report-projection-build.log` 保留早期构建记录；最新表格修复与两个视口的现场结果由主代理确认。下载文件不能由构建代验，本次文档更新不重跑检查 |
...
1. **已验视口边界**：维度表原始 JSON 直接展示问题已修复。1600px 下核验了设备／省份与分期数值；390px 下核验报告正文、历史弹窗及同一维度表，页面无横向溢出，横向滚动后可查看变化列。未逐一检查所有中间断点、数据量或浏览器组合，不将这两种视口扩展为全面视觉验收。
2. **浏览器下载文件**：页面导出按钮已点击，所选 ReAct 报告的版本化接口返回 HTTP 200，但内置浏览器未提供下载事件或可核对的文件；其 CDP 拒绝自定义下载目录。仍需在可捕获文件的环境检查文件名、内容及哈希与同版服务端导出一致；本轮仅服务端固定版本导出通过。
...
5. **交付状态**：代码交付 PR [#200](https://github.com/Jupiter363/shortlink/pull/200) 已合并，merge commit `57615a296f88da216b01e9016d74de51d90a0d33`。本记录确认当前复合主链、固定版本读取／服务端导出、所选后端用例及两个视口的结果；浏览器文件和上述未测场景由 [#201](https://github.com/Jupiter363/shortlink/issues/201) 继续跟踪。
