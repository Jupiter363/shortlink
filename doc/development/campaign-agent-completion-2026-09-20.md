...
| P3-04：持久化 invocation/assistant/toolCall→action/request/hash；规范 PENDING＋READY 新观察，投影版本／序列／ack 与失败重试防丢防重。 | R §1.3；G §4.2 | E26–E28 MODEL/CALL；E29 JDBC轮次与三轮依赖恢复，真实异步PENDING＋唯一READY观察、当前ACL。 | 耐久组件部分已验 | 精确投影ack、预算/上下文规模；补接纳事务故障和未记录PENDING的唯一READY恢复窗口。 |
