...
**最新验收增量（2026-09-24）：公共 Graph、真实 MySQL、Admin→Analytics 与真实模型已完成复合业务和局部 ReAct 真实闭环。** 复合 Run `b9c90398…15ea0` 的 6/6 步骤成功，7 个唯一 query job 无重复，报告 revision 2 为 READY、3/3 目标 ANSWERED；单查询 ReAct Run `dc4fe737…16cf2` 有两轮模型、一次 CALL／一个 job，报告 2/2 ANSWERED。详情与未覆盖范围见 §1.3 和[最终真实验收记录][FINAL-20260924]。前端维度表 JSON 视觉缺陷已修复并通过真实内置浏览器宽屏复验；窄屏尚未验收，不能据此宣称全部验收完成。
...
截至 2026-09-23，E65–E126 已按批次补充实现与定向后端验证；本矩阵只把实际通过的局部组件标为部分已验，未将完整 Graph/HTTP、客户端、Docker 或真实 MySQL 的端到端验收写成完成。

2026-09-24 公共链路集成批次快照：仍实施同一 Issue #62，不新增或替换架构计划。E00–E126 是对应批次的历史证据，表中“尚未接入”的历史说明保留；该批次公共接线与验收状态见[交付记录][E127]，不能把历史组件通过数累计成整体验收。该批次公共链路和通用结果页代码已完成，连接点最小必要后端验收及前端静态检查通过；当时真实环境与视觉待验，后续真实验收见 §1.3。
...
**历史基线结论（截至 E126）：P0 原生机制组件门槛已有验收；P1 多个耐久运行、恢复、分页接收及固定统计执行组件已有后端验收；E123 只在显式 profile 下接通比较／排名的 FIXED 取数入口。E127 现已补齐公共 Plan/ReAct、报告读取和通用结果页接线，后端定向验收通过，真实环境和视觉仍待验。新增 profile 默认关闭。** 本文的“待完成”是任务，不是删除或降低原验收条件。
...
| [E37：原生Skill具名多输出观察][E37] | 两项后端方法首次通过；真实Native下降筛选PENDING→原CALL续接→双输出READY，完整metadata和有限真实事实进入MODEL2；空saver／撤权恢复不重调，旧统计Tool兼容。 | 原生单片异步Skill整链已验；结构化终局验收、多阶段混合恢复、Driver／生产装配及报告仍待完成，CANDIDATE不等于SUCCEEDED。 |
| [E38：结构化候选与后端条件验收][E38] | 三项后端方法通过；真实Native双输出按冻结条件分别COMPLETE或UNKNOWN拒绝，重开同源不重调，旧writer／撤权拒绝；缺输入／重规划／无进展／非法候选耐久化，旧Skill兼容。 | 局部候选合同已验；无Step输出发布或Goal完成，NO_PROGRESS仅为模型报告，Driver／真正停止机制及生产入口继续实现。 |
...
| [E107：准入候选可信执行][E107] | 3 个 E107 直接用例，加上 E102/E104/E106/E107 application 回归共 22 个纯 Java 定向用例通过（`CampaignReplanAdmissionExecutionTest`）；执行顺序固定为 identity→resolver→binder→runtime，裸 candidate、legacy handoff、身份漂移和缺 token 不到 coordinator。 | 仍是 transport-neutral typed executor，尚未注册 HTTP/chat/tool/Graph/Spring 入口或真实 MySQL；accepted/idempotent 事实继续由既有 application/coordinator 合同负责。 |
| [E108：报告模块响应投影][E108] | 31 个受影响 durable response/projection 后端用例通过；`AgentRunResult.Report` 的可选 modules 字段承载 `campaign-report-modules/v1`，旧构造和无模块 JSON 保持兼容，section block→goal 归属在投影边界生成。 | 仍是通用响应的 typed 脱敏字段，尚未接 HTTP 客户端、前端页面、真实 MySQL 或浏览器；歧义 block 归属不降级为可渲染完成。 |
...
| [E113：维度下钻接入复合 child 执行][E113] | `CampaignStatisticsCompositeCallTest` 4 个、`DimensionChangeSkillTest` 2 个定向用例通过；双期间维度子查询统一复用冻结 child/request/wire 身份，READY/WAITING/容量拒绝/提交未确定分层后才允许发布分页产物。 | 只接通真实维度下钻的复合 child 边界，compare/rank 仍未接入持久化 CALL、Graph、HTTP 或真实 MySQL。 |
| [E114：比较结果纯计算边界][E114] | `CampaignStatisticsToolsTest` 34 个定向用例通过；比较差值、期间/对象配对、完整性和零基期规则已移入无网关纯计算类，公开工具只负责取数与封装。 | 只完成计算边界拆分，不改变公开工具协议，也不代表 compare/rank 已接入持久化 CALL、Graph、HTTP 或真实 MySQL。 |
...
| [E126：选中范围的单次来源扫描][E126] | 内部范围重建改为从一次完整链验证的真实来源页流式输出，移除逐选中页重验整链及二次索引读取。501 成员四批输出的来源检查次数保持一次；新增扫描、既有范围边界及完整依赖链共 5 项定向后端用例通过。 | 仅支持规范有序来源分片；旧独立分页保持兼容。维度 progress 重复前缀扫描、公共聊天与报告交付仍待推进，未做真实 MySQL/大集合性能验收。 |
| [E127：公共链路与通用结果页][E127] | 原始提问、多目标规划、FIXED/局部原生探索、依赖推进、自动恢复、跨轮/改版、逐目标报告、历史/分页/导出及前端七类块接通。完整公共依赖 Graph 与原 job 接管定向用例通过，报告版本与当前权限、模型未知结果不重发等分支通过，前端定向静态检查通过。 | H2/脚本模型与静态前端，不代表真实 MySQL、部署重启、跨服务、真实模型质量或视觉通过。接管仅限经完整校验的 FIXED/FROZEN_SET 原任务。 |
...
### 1.2 2026-09-24 公共交付校准

以下六项对应原计划的剩余公共链路，具体证据见 [E127][E127]；不替换第 2–6 节的原始要求，也不重记已经完成的组件工作。

| 原计划范围 | 既有实现 | 本次接通 | 当前验收边界 |
| --- | --- | --- | --- |
| P1/P4 规划入口 | 冻结输入、可信规划、耐久 Run、业务 Plan factory | 公共提问登记、需求提取、多目标与多期间候选校验、统一 intake | 新连接点定向后端验证；真实模型语义质量待验。 |
| P2 依赖业务 | E125/E126 的枚举、比较、下降筛选、联合维度 Skill | 与独立查询共同规划，接入公共 Graph 和逐目标报告 | 完整依赖场景定向验证；历史大集合组件证据复用。 |
| P3 探索及恢复 | 原生 ReactAgent、调用及观察账本、容量与恢复协调器 | 生产 Profile 装配、到期引用扫描、同 Run 去重与退避 | 脚本模型/H2；实际应用重启部署及跨服务环境待验。 |
| P4 跨轮及 revision | receipt、consumer adoption、原子 revision 围栏 | 查询进度、继续、补充/新需求分流；可信探索触发耐久改版 | 仅接管经完整证明的同 stepId FIXED/FROZEN_SET job；不兼容改版拒绝。 |
| P5 交付与读取 | GoalAssessor、发布与 binding、报告生命周期 | 实际证据、耐久解读、逐目标终评、授权历史/分页/导出 | 固定版本及撤权用例；真实 MySQL 隔离/迁移待验。 |
| UI 通用结果页 | 原有 Vue、Element Plus、ECharts 与安全 Markdown | 七类块、逐目标进度、图文章节、原身份继续、历史与导出 | 仅静态接口/状态/组件/样式检查；真实浏览器与视觉待验。 |

### 1.3 2026-09-24 最终真实验收增量

以下为主线程完成的真实环境验收结果，完整证据及早期失败的修复关系见[最终真实验收记录][FINAL-20260924]。它补充 §1.2 的公共接线批次，不将历史组件验证改写为当时已经通过真实环境；本次矩阵更新只核对文档差异，没有另跑测试或 Docker。

| 原计划范围 | 最新真实验收结果 | 当前边界 |
| --- | --- | --- |
| P1/P3/P4 公共运行链 | Run `b9c90398…15ea0` 经公共 Graph、真实 MySQL 持久化、Admin→Analytics 和真实模型完成，6/6 步骤成功；实际共 7 个唯一 query job，无重复提交生成的新 job。 | 证明该真实复合场景的公共闭环，不代表所有提示词、模型输出或故障时序均已验收。 |
| P3 局部 ReAct | Run `dc4fe737…16cf2` 的冻结计划含一个 `REACT / statistics-exploration` 步骤；持久账本记录工具观察后终局的两轮模型、一次已返回 CALL 和一个唯一统计 job。步骤成功，报告 2/2 ANSWERED，第二个解释目标复用相同数据。 | 只覆盖所选分组、单日浏览器维度的真实只读探索；其他 Tool／Skill 和真实重启故障窗口仍需独立验证。 |
| P2 下降筛选与依赖维度 | 下降筛选结果 `[20003,20004]` 作为后续 `province + device` 查询的 `FROZEN_SET`，依赖步骤使用筛选所得冻结集合。 | 不以原全集替代入选集合；未据此声称本次真实数据存在 UNKNOWN 地域样本。 |
| P2 独立全集排名 | 独立排名按其授权候选全集执行，与下降集合上的联合维度查询保持不同范围。 | 不把排名缩成下降子集，也不把这一场景扩大为任意排名组合已验。 |
| P5 报告、历史与刷新 | 正式报告 revision 2 为 READY，3/3 目标 ANSWERED；历史 revision 1 与刷新后读取 revision 2 已验。 | 报告版本分别保留，旧版不是新版替身；步骤成功与逐目标完成分别核验。 |
| P5 分页与导出 | 后端完整结果分页与导出验收通过。 | 浏览器下载文件的捕获、实际字节与内容核验未完成，不标记端到端下载文件验收通过。 |
| UI 通用结果页 | 维度明细已改为设备／省份标签及基期／目标期 PV、UV、UIP、PV 占比，原始复杂字段可展开，表内可横向滚动；修复后已通过真实内置浏览器宽屏视觉复验。 | 窄屏尚未验收，不把宽屏结果扩大为所有视口或完整前端验收通过。 |
...
| 要求 | 来源 | 证据 | 当前状态 | 剩余动作 |
| --- | --- | --- | --- | --- |
| P2-01：CURRENT_GROUP／FROZEN_SET 不混；可信 FrozenScope 保存主体、来源组、完整 members/hash、枚举证明与授权版本；Graph 仅 scopeRef。 | C §3、§5；R §4.1；R14 | E11/E15/E16真实成员凭证，E24原生节点，E25同Plan动态范围接筛选/维度。 | 部分已验 | 生产授权装配与真实环境验收；枚举版本不是历史可重读快照。 |
...
| P2-08：`decline_selection` 真实执行两期全量对齐，delta<0 保留全部且按口径排序；baseline=0 rate=null，缺失／未创建／无遥测不当零。 | C §6、§9.3 | E17–E20全量计算和真实Skill；E25 v2直接消费同Plan权威范围，复用分页/恢复/释放/双输出链，v1保留。 | 固定Skill已实现；后端组件已验 | 生产请求装配和环境验收仍需完成；不以Step成功代替Goal终评。 |
...
以下三项是截至 E126 的历史推进顺序，E127 已按同一目标完成公共接线与本次允许范围内的验证。后续只推进明确开放的真实环境与视觉验收，不重新实施这些已有组件：
...
| 要求 | 来源 | 证据 | 当前状态 | 剩余动作 |
| --- | --- | --- | --- | --- |
| VERIFY-01：必要的受影响后端单元／组件／H2／loopback／脚本模型测试，失败修复后只重跑受影响项。 | I §6、仓库 AGENTS.md、当前用户限制 | E127 逐项记录最终通过方法；E125/E126 未受影响证据复用 | 本次定向验收通过 | 保持最小测试范围，不把方法级通过与失败批次混淆，不重跑无关检查。 |
...
| VERIFY-04：新客户端状态、结果可访问性、图文布局/交互、历史/复制/导出同版。 | I §2、§5–7；UI-01–04 | E127 新协议前端实现及静态检查 | 代码完成／视觉待验 | 本次不跑浏览器、前端构建或应用，不把静态检查写成实际交互验收通过。 |
...
[E37]: ../integration/campaign-plan-p3-native-skill-2026-09-20.md
[E38]: ../integration/campaign-plan-p3-candidate-assessment-2026-09-20.md
...
[E79]: ../integration/campaign-plan-p4-replan-p5-lifecycle-2026-09-20.md
[E80]: ../integration/campaign-plan-p4-replan-p5-lifecycle-2026-09-20.md
...
[E107]: ../integration/campaign-plan-p4-replan-p5-lifecycle-2026-09-20.md
[E108]: ../integration/campaign-plan-p4-replan-p5-lifecycle-2026-09-20.md
...
[E113]: ../integration/campaign-plan-p4-replan-p5-lifecycle-2026-09-20.md
[E114]: ../integration/campaign-plan-p4-replan-p5-lifecycle-2026-09-20.md
