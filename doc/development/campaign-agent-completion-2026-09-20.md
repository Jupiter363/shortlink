...
| [E49：重规划耐久收据][E49] | 一个 H2 定向方法通过；按当前 Run token 锁定 ACTIVE revision，以 `(run,baseRevision)` 幂等保存候选决策，冲突正文/hash 拒绝覆盖，旧 revision 不提前淘汰。 | 尚未接 REQUEST_REPLAN、事务内 Graph/consumer 切版、报告生产生命周期或真实 MySQL；收据表和协调器仍内部未注册。 |
