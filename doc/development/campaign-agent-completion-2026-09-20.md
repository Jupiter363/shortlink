...
| P3-04：持久化 invocation/assistant/toolCall→action/request/hash；规范 PENDING＋READY 新观察，投影版本／序列／ack 与失败重试防丢防重。 | R §1.3；G §4.2 | E26–E28 MODEL/CALL；E29 JDBC轮次与三轮依赖恢复，真实异步PENDING＋唯一READY观察、当前ACL。 | 耐久组件部分已验 | E30已补模型预算和历史引用加载；精确投影ack、接纳事务故障及未记录PENDING的唯一READY恢复窗口待验。 |
...
| P3-06：有限协议修复、无进展、停止／重试／预算按 Run 累计，继续／重启／新 revision 不重置；耗尽保留证据与明确 stopReason。 | R §1.1；C §3.3、§9.5 | E00修复机制；E30模型/CALL/repair逻辑槽和Run累计，模型预算跨重开/升级/版本及上下文字节边界已验。 | 部分已验 | 修复耗尽/无进展、token费用和显式预算调整协议仍待验；COMPLETE候选不能绕后端验收。 |
