...
| P2-09：`dimension_change` 继承确切动态入选集合，两期间真实联合维度；UNKNOWN/NOT_APPLICABLE 分开，PV 全窗分母，UV/UIP 独立去重。 | C §6；R §4.2 | E21真实pair/源期间与501派生范围；既有单对象下钻＋E10 DIMENSION_BREAKDOWN 是底层能力。 | 派生集合组件已验；维度Skill待接 | STEP_OUTPUT动态绑定、真实联合查询、明确cohort的比较与产物；不可拼边际分布或跨片累加UV/UIP。 |
