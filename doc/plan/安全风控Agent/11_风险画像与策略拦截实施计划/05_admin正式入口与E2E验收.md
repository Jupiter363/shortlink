...
- [x] 确认 risk-center internal API 已可用。
- [x] 确认 admin UserContext、GroupService 或等价能力可校验 gid 归属。
- [x] 确认真实本机闭环只通过环境变量注入 `DEEPSEEK_API_KEY`、`RISK_HASH_SALT` 和 internal token。
...
- Modify: `doc/plan/安全风控Agent/05_验收清单.md`
...
```text
当前进程未设置 DEEPSEEK_API_KEY、AGENT_DATASOURCE_URL、AGENT_DATASOURCE_USERNAME、AGENT_DATASOURCE_PASSWORD、RISK_HASH_SALT、AGENT_INTERNAL_TOKEN。
本机 8000、8010、8001、8002 端口未开放。
因此本步骤暂不强行启动整套服务；已提供 scripts/risk-profile-policy-e2e.ps1 作为服务就绪后的可重复手动闭环入口。
```

- [x] **Step 6: 更新验收清单**

在 `doc/plan/安全风控Agent/05_验收清单.md` 增加：
...
```bash
git add agent-service/src/test/java/com/jupiter/shortlink/agent/e2e/RiskProfilePolicyE2eTest.java scripts/risk-profile-policy-e2e.ps1 doc/plan/安全风控Agent/05_验收清单.md
