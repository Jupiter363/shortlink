...
[远端结果释放与配额分账](../../integration/campaign-plan-p2-result-release-2026-09-20.md)已实现服务端释放、原身份保留、容量未受理回执及客户端合同；Agent 的消费登记、releaseIntent、自动释放和未提交子项退避仍待接线，不能将远端协议完成视为 R18 关闭。
