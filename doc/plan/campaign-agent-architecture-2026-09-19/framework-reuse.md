...
状态：**设计决策；P0 部分控制点已验证，完整门槛仍待完成**。2026-09-19；项目固定版本 `1.1.2.3`。实际结果见 [P0 后端验证](../../integration/campaign-plan-p0-2026-09-19.md)。本地 JAR API／字节码用于确认可调用接口，官方文档用于理解能力方向；线上文档出现的新接口不能直接视为本地版本可用。
...
上述列表是完整验收门槛。首批后端验证已覆盖原生循环、等待停止、批次拒绝、状态投影、未决超时和独立容量组件；WAITING→READY 完整恢复、Skill 版本与权限交集、生产共享执行器集成仍未完成，不将首批通过视为整个 P0 关闭。原生 asNode 反例后选择显式投影薄适配；Jackson 仅在 Agent 子模块对齐 graph-core 已声明的版本，详见验证记录。
