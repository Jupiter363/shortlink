...
class RedirectRoutingParityTest {
    ...
    void sampledTimingAndCorrelationSurviveGetHeadAndDeferredBusinessErrors(boolean functional) {
        for (HttpMethod method : List.of(HttpMethod.GET, HttpMethod.HEAD)) {
            try (var f = new Fixture(functional, true)) {
                ...
                f.request(method, "/Ab").header("X-Request-ID", requestId).exchange()
                        .expectStatus().isFound()
                        .expectHeader().valueEquals("X-Request-ID", requestId)
                        .expectHeader().value("X-Shortlink-Handler-Nanos", value -> {
        ...
        try (var f = new Fixture(functional, true)) {
            ...
            when(f.routes.resolve("s.example", "Ab"))
                    .thenReturn(Mono.defer(() -> Mono.error(new IllegalStateException("unavailable"))));
            f.request(HttpMethod.GET, "/Ab").header("X-Request-ID", requestId).exchange()
                    .expectStatus().isEqualTo(503)
                    .expectHeader().valueEquals("X-Request-ID", requestId)
                    .expectHeader().valueMatches("X-Shortlink-Handler-Nanos", "[0-9]+");
            verify(f.events).result(argThat(event -> requestId.equals(event.requestId()) && event.status() == 503));
    ...
    void disabledTimingPreservesCorrelationWithoutAFilterBean(boolean functional) {
        try (var f = new Fixture(functional, false)) {
            ...
            String requestId = "0123456789abcdef0123456789abcdef";
            f.request(HttpMethod.HEAD, "/Ab").header("X-Request-ID", requestId).exchange()
                    .expectStatus().isFound()
                    .expectHeader().valueEquals("X-Request-ID", requestId)
                    .expectHeader().doesNotExist("X-Shortlink-Handler-Nanos");
        }
    }
    ...
    void exclusiveMappingPreservesGetHeadCookiesAndRealObservationTags(boolean functional) {
        try (var f = new Fixture(functional)) {
            assertEquals(functional ? 0 : 1, f.context.getBeansOfType(RedirectAnnotatedRoute.class).size());
            assertEquals(functional ? 1 : 0, f.context.getBeansOfType(RouterFunction.class).size());
            var get = f.request(HttpMethod.GET, "/Ab").exchange().expectStatus().isFound()
                    .expectHeader().location("https://target.example/path")
                    .expectHeader().valueEquals(HttpHeaders.CACHE_CONTROL, "no-store, private")
                    .expectBody().isEmpty();
            ...
            var head = f.request(HttpMethod.HEAD, "/Ab").exchange().expectStatus().isFound()
                    .expectHeader().location("https://target.example/path").expectBody().isEmpty();
            ...
            assertNotEquals(results.getAllValues().get(0).requestId(), results.getAllValues().get(1).requestId());
            assertTrue(EventIdentity.valid(clicks.getValue().eventId(), clicks.getValue().occurredAt()));
            for (var event : results.getAllValues()) assertTrue(EventIdentity.valid(event.decisionId(), event.occurredAt()));
            await(() -> f.meters.find("http.server.requests").tags("method", "GET", "status", "302",
                    "uri", "/{shortUri}").timer() != null);
            assertEquals(1, f.meters.get("http.server.requests").tags("method", "GET", "status", "302",
                    "uri", "/{shortUri}").timer().count());
        }
    }
    ...
    void existingVisitorIsReusedAndHttpReplacementCookieIsNotSecure(boolean functional) {
        try (var f = new Fixture(functional)) {
            ...
            f.request(HttpMethod.GET, "/Ab").cookie("sl_uv", visitor).exchange()
                    .expectStatus().isFound().expectCookie().doesNotExist("sl_uv");
            verify(f.events).click(argThat(e -> visitor.equals(e.uvId())));
            var response = f.request(HttpMethod.GET, "/Ab").cookie("sl_uv", "invalid")
                    .headers(h -> h.set("X-Forwarded-Proto", "http")).exchange()
                    .expectStatus().isFound().expectBody().isEmpty();
    ...
    void invalidSingleSegmentAndUnsafeMethodKeepOriginalResponses(boolean functional) {
        try (var f = new Fixture(functional)) {
            for (String path : List.of("/bad-name", "/" + "a".repeat(33), "/%41b", "/Ab;v=1", "/%2F")) {
            ...
            verify(f.events).result(argThat(e -> e.status() == 405 && "METHOD_NOT_ALLOWED".equals(e.reason())));
        }
    }
    ...
    void hostAndProxyValidationStillRunBeforeAuthorityReads(boolean functional) {
        try (var f = new Fixture(functional)) {
            ...
            f.request(HttpMethod.GET, "/Ab").headers(h -> h.set("X-Forwarded-For", "127.0.0.1"))
                    .exchange().expectStatus().isForbidden();
            f.network.peer = "127.0.0.1";
            f.request(HttpMethod.GET, "/Ab").headers(h -> h.set(HttpHeaders.HOST, "unknown.example"))
                    .exchange().expectStatus().isNotFound();
            f.request(HttpMethod.GET, "/Ab").headers(h -> h.set("X-Forwarded-Proto", "ftp"))
                    .exchange().expectStatus().isBadRequest();
            f.request(HttpMethod.GET, "/Ab").headers(h -> h.set("X-Forwarded-For", "bad-ip"))
                    .exchange().expectStatus().isBadRequest();
            verifyNoInteractions(f.routes, f.policies, f.rates);
            verify(f.events, never()).click(any());
        }
    ...
    void lifecycle404BodyAndSecurityHeadersAreUnchanged(boolean functional) {
        try (var f = new Fixture(functional)) {
            ...
            when(f.routes.resolve("s.example", "Ab")).thenReturn(Mono.just(f.route("DELETED", null)));
            var get = f.request(HttpMethod.GET, "/Ab").exchange().expectStatus().isNotFound()
                    .expectHeader().doesNotExist(HttpHeaders.LOCATION)
                    .expectHeader().valueEquals("Content-Security-Policy", "default-src 'none'; frame-ancestors 'none'")
                    .expectHeader().valueEquals("X-Content-Type-Options", "nosniff")
                    .expectHeader().valueEquals(HttpHeaders.CACHE_CONTROL, "no-store, private")
                    .expectBody().returnResult();
            assertTrue(new String(get.getResponseBody(), StandardCharsets.UTF_8).contains("短链接不可用"));
            f.request(HttpMethod.HEAD, "/Ab").exchange().expectStatus().isNotFound().expectBody().isEmpty();
            f.request(HttpMethod.GET, "/Ab").header(HttpHeaders.ACCEPT, "application/json")
    ...
    void policyDenyAndGlobalQuotaDecisionsAreNotBypassed(boolean functional) {
        try (var f = new Fixture(functional)) {
            ...
            when(f.rates.evaluate(eq("1:5"), any())).thenReturn(Mono.just(new RiskDecision(429, "RATE_LIMITED", 1, null)));
    ...
    void authorityFailureAndFinalRouteProofExpiryStayFailClosed(boolean functional) {
        try (var f = new Fixture(functional)) {
            when(f.policies.resolve("1", 5)).thenReturn(Mono.error(new IllegalStateException("test unavailable")));
            ...
            when(f.rates.evaluate(eq("1:5"), any())).thenAnswer(i -> Mono.fromSupplier(() -> {
                f.now.set(2000); return i.getArgument(1);
            }));
            f.request(HttpMethod.GET, "/Ab").exchange().expectStatus().isEqualTo(503)
                    .expectHeader().doesNotExist(HttpHeaders.LOCATION);
    ...
    void finalPolicyTransitionIsRecheckedAfterQuota(boolean functional) {
        try (var f = new Fixture(functional)) {
            ...
            when(f.rates.evaluate(eq("1:5"), any())).thenAnswer(i -> Mono.fromSupplier(() -> {
                f.now.set(1600); return i.getArgument(1);
            }));
            f.request(HttpMethod.GET, "/Ab").exchange().expectStatus().isEqualTo(503)
                    .expectHeader().doesNotExist(HttpHeaders.LOCATION);
    ...
    void applicationFiltersAndInternalControllerRemainInTheDispatchChain(boolean functional) {
        try (var f = new Fixture(functional)) {
            ...
            f.request(HttpMethod.GET, "/Ab").exchange().expectStatus().isForbidden()
                    .expectHeader().valueEquals("X-Test-Filter", "rejected");
            ...
            f.request(HttpMethod.GET, "/internal/v1/events/quality").exchange().expectStatus().isForbidden();
            f.request(HttpMethod.GET, "/internal/v1/events/quality")
                    .header("X-Internal-Token", TestConfig.defaults().internalToken()).exchange()
                    .expectStatus().isOk().expectBody().json("{\"probe\":true}");
            for (String path : List.of("/internal/other", "/actuator/unmapped", "/Ab/extra", "/Ab/")) {
                f.request(HttpMethod.GET, path).exchange().expectStatus().isNotFound();
            }
            verify(f.events, never()).click(any());
    ...
    void frameworkOptionsAndCorsPreflightDoNotBecomeRedirectEvents(boolean functional) {
        try (var f = new Fixture(functional)) {
            var options = f.request(HttpMethod.OPTIONS, "/Ab").exchange().expectStatus().isOk()
                    .expectBody().isEmpty();
            assertEquals(Set.of(HttpMethod.GET, HttpMethod.HEAD, HttpMethod.POST, HttpMethod.PUT,
                    HttpMethod.PATCH, HttpMethod.DELETE, HttpMethod.OPTIONS), options.getResponseHeaders().getAllow());
            f.request(HttpMethod.OPTIONS, "/Ab").header(HttpHeaders.ORIGIN, "https://foreign.example")
    ...
    private static final class Fixture implements AutoCloseable {
        ...
        Fixture(Boolean functional, Boolean timing) {
            if (functional != null) context.getEnvironment().getPropertySources().addFirst(
                    new MapPropertySource("routing-test", Map.of("shortlink.redirect.functional-routing-enabled", functional)));
            if (timing != null) context.getEnvironment().getPropertySources().addFirst(
                    new MapPropertySource("timing-test", Map.of("shortlink.redirect.request-timing-enabled", timing)));
            Clock clock = mock(Clock.class);
            when(clock.millis()).thenAnswer(i -> now.get());
            when(routes.resolve("s.example", "Ab")).thenReturn(Mono.just(route("ACTIVE", "https://target.example/path")));
            ...
            observation.observationConfig().observationHandler(new DefaultMeterObservationHandler(meters));
            ...
            context.registerBean(ServerHttpObservationFilter.class, () -> new ServerHttpObservationFilter(observation));
            context.register(WebConfiguration.class, RedirectController.class, RedirectAnnotatedRoute.class,
                    RedirectFunctionalRouting.class, EventQualityController.class, RedirectRequestTimingFilter.class);
            context.refresh();
        ...
        WebTestClient.RequestBodySpec request(HttpMethod method, String rawPath) {
            return client.method(method).uri(URI.create("http://s.example" + rawPath))
        ...
        RouteInfo route(String status, String target) {
            return new RouteInfo("s.example", "Ab", 5, "1", "g", target, status, null, 1, 1, 1000, 2000, 1);
        }

        PolicySnapshot policy(boolean disabled, long until) {
            return new PolicySnapshot("1:5", 1, 1000, until == 1600 ? 1600L : null, until,
                    disabled ? PolicyState.KNOWN_RESTRICTED : PolicyState.KNOWN_ALLOWED,
                    disabled, true, "UTC", List.of(), Set.of(), null);
        }
    ...
    /** Mock connector cannot choose a socket peer; the test filter supplies only that transport fact. */
    private static final class NetworkFixtureFilter implements WebFilter, Ordered {
        ...
        public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
            ...
            var request = new ServerHttpRequestDecorator(exchange.getRequest()) {
                @Override public InetSocketAddress getRemoteAddress() { return new InetSocketAddress(peer, 1234); }
            };
